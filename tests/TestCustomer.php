...
class TestCustomer extends TestCase
{
    ...
    public function test_get_distance_from_top(){
        /*
         * QUERY :  SELECT COUNT(*) AS Distance
         *          FROM Queue
         *          WHERE ServiceID = '{$ticket_info["serviceID"]}' AND TicketNumber < '{$ticket_info["ticketN"]}
         * QUEUE | `ID` | `ServiceID` | `TicketNumber` | `Timestamp`
        ...
        $this->assertEquals(1,$dist,"TestCustomer : test_get_distance_from_top wrong returned value");
        $_SESSION['ticketN'] = 0;
        $_SESSION['serviceID'] = 2;
        $dist = get_distance_from_top();
        $this->assertEquals(0,$dist,"TestCustomer : test_get_distance_from_top wrong returned value");
    }

    public function test_get_ticket_html(){
        //todo when the method is complete
        $_SESSION['ticketN']=1;
        $_SESSION['service']="Packages";
        $_SESSION['serviceID']=1;
        $timestamp = mktime(11,00,00,10,17,2019);
        $_SESSION['timestamp'] = $timestamp;
        $format_ticket = "P";
        $format_ticket .= sprintf("%03d", $_SESSION["ticketN"]);
        $cur_ticket = get_currently_served_ticket_by($_SESSION["service"]);
    ...
    <ul>
        ...
        <li class="fo_row"> <span class="passenger" data="Choosen service:">Packages</span></li>
        <li class="fi_row"> 
        ...
        $expected =  $metarefresh . $html_ticket;
        $actual = get_ticket_html();
        $this->assertEquals($expected,$actual,"TestCustomer : test_get_distance_from_top wrong returned value");
